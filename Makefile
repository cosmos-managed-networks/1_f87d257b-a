...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=1 \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=1 \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/1 ./cmd/1
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/1.exe ./cmd/1
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/1
...
draw-deps:
	...
	@goviz -i ./cmd/1 -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c 1 --local -f chains.yaml
endif
...
set-testnet-configs:
	1 config set client chain-id chainid-1
	1 config set client keyring-backend test
	1 config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | 1 keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | 1 keys add acc1 --recover`
